...
async def analyze_text(request: TextIn):
    ...
    result = await analysis_service.analyze_text_service(request.text)
    if not result:
...
async def analyze_url(request: UrlIn):
    ...
    result = await analysis_service.analyze_url_service(str(request.url))
    if not result:
...
async def analyze_image(file: UploadFile = File(...)):
    ...
    result = await analysis_service.analyze_image_service(contents)
    if not result:
...
async def analyze_voice(file: UploadFile = File(...)):
    ...
    result = await analysis_service.analyze_voice_service(file_path)
    if "error" in result:
